...
function forceRemoveHover() {
    ...
    allButtons.forEach(btn => {
        btn.classList.remove('hover', 'hover-active', 'active');
        btn.blur();
...
function showChapterPage(data) {
    ...
    for (let chapterKey in chaptersData) {
        ...
        if (gradeKey === '中考') {
            button.innerHTML = `${chapter.name}<br><small>${chapter.中考标记.join('、')}板块</small>`;
        } else {
